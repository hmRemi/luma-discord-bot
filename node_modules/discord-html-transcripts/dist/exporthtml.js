...
async function generateTranscript(messages, inputChannel, opts = { returnType: 'buffer', fileName: 'transcript.html' }) {
    ...
    await Promise.all(messagesArray.map(async (message) => {
        ...
        if (message.reference?.messageId) {
            ...
            const referencedMessage = messages instanceof userDiscord.Collection ? messages.get(message.reference.messageId) : messages.find(m => m.id === message.reference.messageId);
        ...
        if (message.attachments && message.attachments.size > 0) {
            for (const attachment of Array.from(message.attachments.values())) {
                ...
                if (['png', 'jpg', 'jpeg', 'gif'].includes(attachmentType)) {
                    ...
                    attachmentImage.src = (opts.saveImages ? await (0, utils_1.downloadImageToDataURL)(attachment.proxyURL ?? attachment.url) : null) ?? attachment.proxyURL ?? attachment.url;
    ...
    if (opts.returnType === "attachment")
        return new Attachment(Buffer.from(serialized), opts.fileName ?? 'transcript.html');
