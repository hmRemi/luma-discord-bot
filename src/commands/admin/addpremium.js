...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('add-premium')
        .setDescription('Give a server premium.'),
    owner: true,
    async execute(interaction) {
        ...
        if(premiumProfile) {
            return await interaction.reply({
                embeds: [Embed.setDescription(`${interaction.guild.name} already has premium features.`)]
            });
        }
        ...
        await interaction.reply({
            embeds: [Embed.setDescription(`Premium features have been given to ${interaction.guild.id}`)]
        });
    }
}